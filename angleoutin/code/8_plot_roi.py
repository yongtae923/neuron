# D:\yongtae\neuron\angleoutin\code\8_plot_roi.py

"""
gradient_allen_plot_v2.py

Interactive 3-slice viewer for summary outputs generated by 7_allen_roi.py.

- Input: angleoutin/data/400us_30V_OUT10_IN20/7_gradient_allen/v2_allpoints_gradient_*x_cell*.npy
...
DEFAULT_DATA_ROOT = Path(r"D:\yongtae\neuron\angleoutin\data")
DEFAULT_CASE_NAME = "400us_30V_OUT10_IN20"
...
def _find_inputs_v2(script_dir: Path, case_name: str, cell_id: str | None = None) -> List[Path]:
...
def _vm_by_t_scale(vol: np.ndarray) -> Tuple[float, float, str]:
    finite = vol[np.isfinite(vol)]
    if finite.size == 0:
...
def show_interactive(dataset_items: List[Tuple[str, Path, dict]]) -> None:
    ...
    def update_view(_=None):
        ...
        if metric == "vm_by_t":
            vmin, vmax, cmap_name = _vm_by_t_scale(vol)
        else:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path(s) to v2 summary npy. Comma-separated allowed. Default: load all v2_* in ../data/400us_30V_OUT10_IN20/7_gradient_allen",
    )
    parser.add_argument(
        "--cell",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=f"Case name under angleoutin/data (default: {DEFAULT_CASE_NAME}).",
    )
    parser.add_argument(
        "--data-root",
        type=str,
    ...
    case_name = args.case or os.environ.get("ANGLEOUTIN_CASE", DEFAULT_CASE_NAME)
    data_root = Path(args.data_root)
    case_dir = data_root / case_name
    outdir = case_dir / "7_gradient_allen"

    if not case_dir.is_dir():
        print(f"[ERROR] Case directory not found: {case_dir}")
        raise SystemExit(1)
    if not outdir.is_dir():
        print(f"[ERROR] Missing output directory: {outdir}")
        print("[INFO] Run 7_allen_roi.py first for this case.")
        raise SystemExit(1)

    if args.input:
        input_paths = [Path(p.strip()) for p in str(args.input).split(",") if p.strip()]
    else:
        input_paths = _find_inputs_v2(script_dir, case_name=case_name, cell_id=args.cell)

    if not input_paths:
        msg = (
            "v2 input file not found. Run 7_allen_roi.py first. "
            f"Check {outdir / 'v2_allpoints_gradient_*x_cell*.npy'}"
        )
        if args.cell:
