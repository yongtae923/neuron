"""
gradient_allen_plot.py

Interactive 3-slice viewer for summary outputs generated by gradient_allen.py.

- Input: data/gradient_allen/allpoints_gradient_*x_cell*.npy
...
def _find_input_by_gain(script_dir: Path, gain: float) -> Path | None:
    """Pick the most recently modified summary file for a specific gain."""
    outdir = script_dir / "data" / "gradient_allen"
    if not outdir.is_dir():
        return None

    candidates = list(outdir.glob(f"allpoints_gradient_{_gain_tag(gain)}x_cell*.npy"))
    if not candidates:
        return None
...
def show_interactive(payload: dict, input_path: Path) -> None:
    ...
    xi0 = _nearest_idx(xu, 85.0)
    yi0 = _nearest_idx(yu, 40.0)
    zi0 = nz // 2
    ...
    def _fmt_stats(metric: str, vol: np.ndarray, xi: int, yi: int, zi: int) -> str:
        ...
        lines = [
            ...
            f"metric: {metric}",
            f"gain: {payload.get('gradient_gain', 'N/A')}",
            "",
    ...
    def update_view(_=None):
        ...
        bad_color = "white" if metric == "spike_count" else "black"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "gradient_allen.py summary 결과를 3-slice 슬라이더 뷰어로 시각화하고, "
            "라디오 버튼으로 metric(vm0/vmax/vmin/spike)을 전환합니다."
        )
    )
    parser.add_argument(
        "--input",
        type=str,
        default=None,
        help="Path to summary npy. Default: latest matching file in ./data/gradient_allen",
    )
    parser.add_argument(
        "-e",
        "--exp",
        type=int,
        default=0,
        help="Use summary generated with gain=10^exp (default: 0 -> gain 1).",
    )
    ...
    if args.input:
        ...
        input_path = _find_input_by_gain(script_dir, 10.0 ** int(args.exp))
    if input_path is None:
        raise SystemExit(
            f"Input file not found for gain=10^{int(args.exp)}. "
            "Check ./data/gradient_allen/allpoints_gradient_*x_cell*.npy"
        )
