...
#   4) Remove scale sweep (fixed E_FIELD_SCALE=1.0 only)
...
# E-field scale (fixed, requested: no sweep)
E_FIELD_SCALE = 1.0
...
def worker_run(
    ...
    def get_E_at_index(E: np.ndarray, spatial_idx_all: int, ti: int) -> Tuple[float, float, float]:
        ...
        if E.shape[0] == 3:
            Ex = float(E[0, spatial_idx_all, ti]) * E_FIELD_SCALE
            Ey = float(E[1, spatial_idx_all, ti]) * E_FIELD_SCALE
            Ez = float(E[2, spatial_idx_all, ti]) * E_FIELD_SCALE
        elif E.shape[0] == 2:
            Ex = float(E[0, spatial_idx_all, ti]) * E_FIELD_SCALE
            Ey = 0.0
            Ez = float(E[1, spatial_idx_all, ti]) * E_FIELD_SCALE
        else:
...
def main() -> None:
    ...
    overall = tqdm(total=len(chunks_idx), desc="Workers done", position=0, leave=True, ncols=90)
    ...
    out_name = f"allpoints_cell{CELL_ID}_outsideOnly_integrated_mp_summary_fast_{ts}.npy"
    ...
    final_payload: Dict[str, Any] = {
        ...
        "efield_scale": float(E_FIELD_SCALE),
