...
def _run_for_cell(
    ...
    for grad_gain in gains:
        ...
        out_name = f"v2_allpoints_gradient_{gain_tag}x_cell{cell_id}.npy"
...
def main() -> None:
    ...
    roi_mask = (
        (coords_out_um[:, 0] >= ROI_BOUNDS_UM["x_min"])
        & (coords_out_um[:, 0] <= ROI_BOUNDS_UM["x_max"])
        & (coords_out_um[:, 1] >= ROI_BOUNDS_UM["y_min"])
        & (coords_out_um[:, 1] <= ROI_BOUNDS_UM["y_max"])
        & (coords_out_um[:, 2] >= ROI_BOUNDS_UM["z_min"])
        & (coords_out_um[:, 2] <= ROI_BOUNDS_UM["z_max"])
    )
    positions_all = coords_out_um[roi_mask]
    if positions_all.shape[0] == 0:
        raise RuntimeError("No target points in ROI after outside-coil filtering.")
