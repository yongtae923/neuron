...
Input:
  - efield/600us_50Hz_10umspaing_100mA/grad_Exdx_Eydy_Ezdz_2cycle.npy  (3, Nx, Ny, Nz, Nt)
  - efield/600us_50Hz_10umspaing_100mA/E_field_grid_coords.npy          (N_spatial, 3) in meters

Output (summary only):
  - data/4_output/600us_50Hz_10umspaing_100mA/*.npy

주의: 셀은 529898751만 활성화됨.

사용 예 (WSL):
    wsl
    cd /mnt/d/yongtae/neuron
    conda activate neuronconda
    python ./code/4_simulate_roi.py
------------------------------------------------------------
"""
...
# Ensure project root can be imported when running from code/ or other cwd.
...
if TYPE_CHECKING:  # 타입 체커 전용: 실제 시그니처는 무시
    ...
    from model_allen_neuron import AllenNeuronModel as AllenNeuronModelRuntime
    AllenNeuronModel_t = AllenNeuronModelRuntime  # 런타임에서는 실제 클래스 할당
...
# ROI 전용: 하나 셀만 사용
CELL_IDS = ("529898751",)  # 나머지는 주석 처리
# CELL_IDS = ("529889679",)  # 주석 처리
# CELL_IDS = ("626170547",)  # 주석 처리
# CELL_IDS = ("497232735",)  # 주석 처리

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
BASE_DIR = os.path.dirname(SCRIPT_DIR)  # D:\yongtae\neuron

GRAD_VALUES_FILE = os.path.join(
    BASE_DIR, "efield", "400us_50Hz_10umspaing_100mA", "1x_100mA_grad_Exdx_Eydy_Ezdz_2cycle.npy"
)
E_GRID_COORDS_FILE = os.path.join(
...
def tmp_dir_for_cell(cell_id: str) -> str:
    d = os.path.join(OUTPUT_DIR, f"_tmp_gradient_allen_cell{cell_id}")
...
def worker_run(
    ...
    nt = t_vec.size
    vm_trace_mask = (t_vec >= VM_TRACE_T0_MS - 1e-12) & (t_vec <= VM_TRACE_T1_MS + 1e-12)
    vm_trace_t = t_vec[vm_trace_mask].astype(np.float64)
    vm_trace_idx = np.full(nt, -1, dtype=np.int64)
    vm_trace_idx[np.flatnonzero(vm_trace_mask)] = np.arange(vm_trace_t.size, dtype=np.int64)
    ...
    for li in range(nlocal):
        ...
        for k in range(nt):
            ...
            try:
                ...
                vext = 0.0
            vm = vin - vext
            trace_col = int(vm_trace_idx[k])
            if trace_col >= 0:
                vm_trace[li, trace_col] = vm
...
# =========================
# Run for one cell (module-level for multiprocessing spawn)
# =========================
...
def _run_for_cell(
    ...
    """Run simulation for one cell. Module-level for Process(target=...) pickling."""
    import multiprocessing as _mp

    tmp_dir = tmp_dir_for_cell(cell_id)
    ...
    for grad_gain in gains:
        ...
        overall = tqdm(
            ...
            desc=f"Cell {cell_id} ({grad_gain:g}x)",
            position=0,
            leave=True,
            ncols=90,
        )
        ...
        spike_times_ms: List[List[float]] = [[] for _ in range(npos)]
        vm_trace = np.full((npos, vm_trace_t.size), np.nan, dtype=np.float64)
        ...
        for p in tmp_paths:
            ...
            st_c = np.asarray(payload["spike_times_ms"], dtype=object)
            vm_trace_t_c = np.asarray(payload["vm_trace_t_ms"], dtype=np.float64)
            vm_trace_c = np.asarray(payload["vm_trace_0to1ms_mV"], dtype=np.float64)
            if vm_trace_t_c.shape != vm_trace_t.shape or not np.allclose(vm_trace_t_c, vm_trace_t):
                raise ValueError("Worker vm_trace_t_ms mismatch.")
            ...
            for j in range(idx_chunk.size):
                ...
                spike_times_ms[row] = list(st_c[j])
                vm_trace[row, :] = vm_trace_c[j, :]
        ...
        final_payload: Dict[str, Any] = {
            ...
            "definition": {
                ...
                "spike_times_ms": f"all upward-crossing times (ms) of Vm through {SPIKE_THR_MV} mV",
                "vm_trace_t_ms": f"time axis for Vm trace ({VM_TRACE_T0_MS} to {VM_TRACE_T1_MS} ms)",
                "vm_trace_0to1ms_mV": "Vm trace for each point over vm_trace_t_ms",
            },
    ...
    if not keep_tmp:
        ...
        print(f"[INFO] keep_tmp=True, temp dir kept: {tmp_dir}", flush=True)
...
def main() -> None:
    ...
    ap.add_argument(
        "--sequential",
        action="store_true",
        help="Run cells sequentially instead of in parallel.",
    )
    args = ap.parse_args()
    ...
    if args.workers is not None and int(args.workers) > 0:
        ...
        n_workers = 20
    n_workers = min(n_workers, cpu_total)

    subset_depth = max(0, int(args.subset_depth))

    if args.cell:
        cell_ids = [c.strip() for c in str(args.cell).split(",") if c.strip()]
    else:
    ...
    print(
        "ROI (um): "
        f"x[{ROI_BOUNDS_UM['x_min']:.1f},{ROI_BOUNDS_UM['x_max']:.1f}], "
        f"y[{ROI_BOUNDS_UM['y_min']:.1f},{ROI_BOUNDS_UM['y_max']:.1f}], "
        f"z[{ROI_BOUNDS_UM['z_min']:.1f},{ROI_BOUNDS_UM['z_max']:.1f}]",
        flush=True,
    )
    ...
    # Use all points (no ROI filtering)
    ...
    # Filter out cells that already have all results
    ...
    for cid in cell_ids:
        ...
        for g in gains:
            ...
            if not os.path.exists(out_path):
                ...
                break
        if all_exist:
            print(f"Skip cell {cid}: output already exists", flush=True)
        else:
            cells_to_run.append(cid)

    if not cells_to_run:
        print("All cells already have results. Nothing to do.", flush=True)
        return
    ...
    if args.sequential:
        for cid in cells_to_run:
            _run_for_cell(cell_id=cid, **run_kwds)
    else:
        procs = [
            mp.Process(target=_run_for_cell, args=(cid,), kwargs=run_kwds)
            for cid in cells_to_run
        ]
        for p in procs:
            p.start()
