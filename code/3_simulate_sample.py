# D:\yongtae\neuron\code\3_simulate_sample.py
"""
사용 예 (WSL 환경):
    wsl
...
Input:
    - efield/30V_OUT10_IN20_CI/2gradient.npz, with:
...
# Current gradient source generated by code/2gradient.py
...
def simulate_for_gains(gains: List[float]) -> None:
    ...
    grad_npz = np.load(GRADIENT_NPZ_FILE, allow_pickle=True)
    if "G" not in grad_npz:
        raise KeyError(f"{GRADIENT_NPZ_FILE} has no 'G' array")
    if not all(k in grad_npz for k in ("x_um", "y_um", "z_um", "t_us")):
        raise KeyError(f"{GRADIENT_NPZ_FILE} must contain x_um, y_um, z_um, t_us")

    # 2gradient format: (T, Nx, Ny, Nz, 3) -> internal format: (3, Nx, Ny, Nz, T)
    G_txyzc = grad_npz["G"]
    if G_txyzc.ndim != 5 or G_txyzc.shape[-1] != 3:
        raise ValueError(
            f"Unexpected G shape in {GRADIENT_NPZ_FILE}: {G_txyzc.shape}, expected (T, Nx, Ny, Nz, 3)"
        )
    G = np.transpose(G_txyzc, (4, 1, 2, 3, 0)).astype(np.float64, copy=False)

    x_um = grad_npz["x_um"].astype(np.float64, copy=False)
    y_um = grad_npz["y_um"].astype(np.float64, copy=False)
    z_um = grad_npz["z_um"].astype(np.float64, copy=False)
    t_us = grad_npz["t_us"].astype(np.float64, copy=False)

    # Build Cartesian coordinate list in meters from axis arrays.
    xx, yy, zz = np.meshgrid(x_um * 1e-6, y_um * 1e-6, z_um * 1e-6, indexing="ij")
    coords_m = np.column_stack([xx.ravel(), yy.ravel(), zz.ravel()])
    ...
    for gain in gains:
        ...
        payload = {
            ...
            "gradient_values_file": str(GRADIENT_NPZ_FILE),
            "efield_grid_file": "generated_from_npz_axes",
