...
- Input: efield/30V_OUT10_IN20_CI/4_v2_allpoints_gradient_*x_cell*.npy
...
DEFAULT_INPUT_PATH = BASE_DIR / "efield" / "30V_OUT10_IN20_CI" / "4_cell529898751_1_100mA_allpoints_gradient.npy"
...
def _find_inputs_v2(script_dir: Path, cell_id: str | None = None) -> List[Path]:
    ...
    outdir = script_dir / "efield" / "30V_OUT10_IN20_CI"
    if not outdir.is_dir():
        return []

    if cell_id:
        candidates = list(outdir.glob(f"4_cell{cell_id}_*_100mA_allpoints_gradient.npy"))
    else:
        candidates = list(outdir.glob("4_cell*_*_100mA_allpoints_gradient.npy"))
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path(s) to summary npy. Comma-separated allowed. Default: efield/30V_OUT10_IN20_CI/4_cell529898751_1_100mA_allpoints_gradient.npy",
    )
    parser.add_argument(
        "--cell",
        type=str,
    ...
    if not input_paths:
        msg = (
            ...
            "Check ./efield/30V_OUT10_IN20_CI/4_cell*_*_100mA_allpoints_gradient.npy"
        )
        if args.cell:
