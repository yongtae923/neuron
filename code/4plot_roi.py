...
- Input: data/4_output/600us_50Hz_10umspaing_100mA/v2_allpoints_gradient_*x_cell*.npy
...
DEFAULT_INPUT_PATH = BASE_DIR / "data" / "4_output" / "400us_50Hz_10umspaing_100mA" / "cell529898751_1_100mA_allpoints_gradient.npy"
...
def _find_inputs_v2(script_dir: Path, cell_id: str | None = None) -> List[Path]:
    ...
    outdir = script_dir / "data" / "4_output" / "400us_50Hz_10umspaing_100mA"
    if not outdir.is_dir():
        return []

    if cell_id:
        candidates = list(outdir.glob(f"cell{cell_id}_*_100mA_allpoints_gradient.npy"))
    else:
        candidates = list(outdir.glob("cell*_*_100mA_allpoints_gradient.npy"))
...
def show_interactive(dataset_items: List[Tuple[str, Path, dict]]) -> None:
    ...
    for label, path, payload in dataset_items:
        ...
        # vm_by_t is time-dependent; color limits will be computed dynamically in update_view
        scale_cache["vm_by_t"] = (None, None, "RdBu_r")
        # spike_on is binary; keep a fixed colormap range for clarity
    ...
    def update_view(_=None):
        ...
        # For time-dependent metric vm_by_t compute color limits from current volume
        if metric == "vm_by_t":
            finite = vol[np.isfinite(vol)]
            if finite.size == 0:
                vmin, vmax = -1.0, 1.0
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path(s) to summary npy. Comma-separated allowed. Default: data/4_output/400us_50Hz_10umspaing_100mA/cell529898751_1_100mA_allpoints_gradient.npy",
    )
    parser.add_argument(
        "--cell",
        type=str,
    ...
    if args.input:
        ...
        input_paths = _find_inputs_v2(BASE_DIR, cell_id=args.cell)

    if not input_paths:
        msg = (
            "Input file not found. Run 4_simulate_roi.py first. "
            "Check ./data/4_output/400us_50Hz_10umspaing_100mA/cell*_*_100mA_allpoints_gradient.npy"
        )
        if args.cell:
