# D:\yongtae\neuron\gradientornot\code\1_extract.py

"""
기능:
- gradientornot/data 아래 각 폴더의 0_Ex/0_Ey/0_Ez 텍스트 파일에서
  E-field와 좌표를 추출하고 검증합니다.
- 각 폴더의 0_spec.json에서 설정값을 읽습니다.

입출력:
- 입력: gradientornot/data/<folder_name>/0_{Ex,Ey,Ez}/*.txt
- 입력: gradientornot/data/<folder_name>/0_spec.json
- 출력: gradientornot/data/<folder_name>/1_efield.npy, 1_coords.npy
...
DATA_BASE_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), "data")

# 멀티프로세싱 설정: 전체 코어 중 1개 남기고 나머지 사용
TOTAL_CPUS = cpu_count()
NUM_WORKERS = max(1, TOTAL_CPUS - 1)  # 최소 1개는 보장
BATCH_SIZE = 10  # 배치 단위로 처리할 파일 개수

print(f"CPU 코어: {TOTAL_CPUS}개, 사용: {NUM_WORKERS}개 (1개 남김)")

# --- 2. 헬퍼 함수: 0_spec.json 읽기 ---
def load_spec(spec_path: str) -> Dict[str, Any]:
    """0_spec.json에서 설정값을 읽습니다."""
    if not os.path.exists(spec_path):
        print(f"   경고: {spec_path}이 없습니다. 기본값 사용 (per_cycle=201)")
        return {"per_cycle": 201}
    
    try:
        with open(spec_path, 'r') as f:
            data = json.load(f)
        per_cycle = data.get("data_files", {}).get("per_cycle", 201)
        return {"per_cycle": per_cycle}
    except Exception as e:
        print(f"   경고: 0_spec.json 읽기 오류: {e}. 기본값 사용 (per_cycle=201)")
        return {"per_cycle": 201}


# --- 3. 체크포인트 관리 함수 (TEMP_DIR 매개변수 추가) ---
def get_checkpoint_path(temp_dir: str, component_name: str) -> str:
    """체크포인트 파일 경로 반환"""
    return os.path.join(temp_dir, f"checkpoint_{component_name}.json")

def load_checkpoint(temp_dir: str, component_name: str) -> list:
    """체크포인트 로드 (처리된 배치 인덱스 리스트 반환)"""
    checkpoint_path = get_checkpoint_path(temp_dir, component_name)
...
def save_checkpoint(temp_dir: str, component_name: str, processed_batches: list) -> None:
    """체크포인트 저장"""
    checkpoint_path = get_checkpoint_path(temp_dir, component_name)
    with open(checkpoint_path, 'w') as f:
        json.dump({'processed_batches': processed_batches}, f)

def get_batch_file_path(temp_dir: str, component_name: str, batch_idx: int) -> str:
    """배치 임시 파일 경로 반환"""
    return os.path.join(temp_dir, f"{component_name}_batch_{batch_idx:03d}.npy")

def clear_checkpoint(temp_dir: str, component_name: str) -> None:
    """체크포인트 및 임시 파일 삭제"""
    checkpoint_path = get_checkpoint_path(temp_dir, component_name)
    if os.path.exists(checkpoint_path):
...
# --- 5. 배치 단위 데이터 로드 함수 ---
def load_e_field_component(component_dir: str, temp_dir: str, total_steps: int = 201, batch_size: int = 20) -> NDArray[np.float32]:
    """
    배치 단위로 파일을 로드하고 체크포인트를 사용하여 재시작 가능하게 처리합니다.
    
    Args:
        component_dir: E-field 성분 폴더 경로 (0_Ex, 0_Ey, 또는 0_Ez)
    ...
    # 체크포인트 로드
    processed_batches = load_checkpoint(temp_dir, component_name)
    ...
    for batch_idx in range(num_batches):
        ...
        batch_file_path = get_batch_file_path(temp_dir, component_name, batch_idx)
        ...
        save_checkpoint(temp_dir, component_name, processed_batches)
    ...
    # 임시 파일 및 체크포인트 정리
    clear_checkpoint(temp_dir, component_name)
...
# --- 5. 케이스별 처리 함수 ---
def process_case(case_folder_path: str, case_name: str) -> bool:
    """
    한 개의 케이스 폴더를 처리합니다.
    
    Args:
        case_folder_path: 케이스 폴더 경로 (예: .../data/30V_OUT10_IN20_CI)
        case_name: 케이스 이름
    ...
    """
    print("\n" + "=" * 60)
    print(f"케이스 처리: {case_name}")
    print("=" * 60)
    
    # 0_spec.json에서 설정값 읽기
    spec_path = os.path.join(case_folder_path, "0_spec.json")
    spec = load_spec(spec_path)
    STEPS_PER_CYCLE = spec["per_cycle"]
    NUM_CYCLES = 1
    TOTAL_STEPS = STEPS_PER_CYCLE * NUM_CYCLES
    
    print(f"설정: STEPS_PER_CYCLE={STEPS_PER_CYCLE}, NUM_CYCLES={NUM_CYCLES}, TOTAL_STEPS={TOTAL_STEPS}")
    
    # E-field 폴더 경로 (0_Ex, 0_Ey, 0_Ez)
    EX_DIR = os.path.join(case_folder_path, "0_Ex")
    EY_DIR = os.path.join(case_folder_path, "0_Ey")
    EZ_DIR = os.path.join(case_folder_path, "0_Ez")
    
    # 출력 파일 경로 (1_efield.npy, 1_coords.npy)
    OUTPUT_E_FIELD_FILE = os.path.join(case_folder_path, "1_efield.npy")
    OUTPUT_COORDS_FILE = os.path.join(case_folder_path, "1_coords.npy")
    
    # 임시 파일 디렉토리
    TEMP_DIR = os.path.join(case_folder_path, "_temp_extract")
    os.makedirs(TEMP_DIR, exist_ok=True)
    
    # 디렉토리 존재 확인
    for dir_name, dir_path in [("0_Ex", EX_DIR), ("0_Ey", EY_DIR), ("0_Ez", EZ_DIR)]:
        if not os.path.exists(dir_path):
            print(f"   오류: {dir_name} 디렉토리를 찾을 수 없습니다: {dir_path}")
            return False
    
    print(f"경로: {case_folder_path}")
    print(f"0_Ex: {EX_DIR}")
    print(f"0_Ey: {EY_DIR}")
    print(f"0_Ez: {EZ_DIR}")
    print(f"출력: {OUTPUT_E_FIELD_FILE}, {OUTPUT_COORDS_FILE}")
    ...
    try:
        ...
        E_x = load_e_field_component(EX_DIR, TEMP_DIR, STEPS_PER_CYCLE, BATCH_SIZE)
        E_y = load_e_field_component(EY_DIR, TEMP_DIR, STEPS_PER_CYCLE, BATCH_SIZE)
        E_z = load_e_field_component(EZ_DIR, TEMP_DIR, STEPS_PER_CYCLE, BATCH_SIZE)
        
        # Shape 확인
        if not (E_x.shape[0] == E_y.shape[0] == E_z.shape[0]):
            print(f"오류: 공간 지점 수가 일치하지 않습니다: "
                  f"Ex={E_x.shape[0]}, Ey={E_y.shape[0]}, Ez={E_z.shape[0]}")
            return False
        
        # 반복 처리 (NUM_CYCLES > 1인 경우)
        if NUM_CYCLES > 1:
...
if __name__ == "__main__":
    print("=" * 60)
    print("E-field 데이터 추출 (다중 폴더)")
    print("=" * 60)
    print(f"DATA_BASE_DIR: {DATA_BASE_DIR}\n")
    
    # gradientornot/data 아래의 모든 폴더 찾기
    if not os.path.exists(DATA_BASE_DIR):
        print(f"오류: {DATA_BASE_DIR}이 존재하지 않습니다.")
        exit(1)
    
    case_folders = sorted([
        d for d in os.listdir(DATA_BASE_DIR)
        if os.path.isdir(os.path.join(DATA_BASE_DIR, d))
    ])
    
    if not case_folders:
        print(f"오류: {DATA_BASE_DIR} 아래에 폴더가 없습니다.")
        exit(1)
    
    print(f"찾은 케이스 폴더 ({len(case_folders)}개):")
    ...
    failed = 0
    for case_name in case_folders:
        case_path = os.path.join(DATA_BASE_DIR, case_name)
