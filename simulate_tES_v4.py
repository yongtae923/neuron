...
# === E-field 단위 변환 설정 ===
# 1. FEM 데이터 단위 → V/m 변환 (데이터 로드 시 적용)
#    - FEM이 V/m인 경우: 1.0
#    - FEM이 V/mm인 경우: 1000.0
#    - FEM이 V/um인 경우: 1e6
E_UNIT_SCALE = 1e6  # V/um → V/m

# 2. (V/m × um) → mV 변환 (계산 시 적용)
#    phi = E(V/m) × d(um) × E_factor → mV
#    E_factor = 1e-6 (um→m) × 1e3 (V→mV) = 1e-3
E_FACTOR = 1e-3

# === E-field 강도 스케일 설정 (정규화 없음, 원본 기준 배수) ===
# 단일 스케일: E_FIELD_SCALES = [1.0]
# 여러 스케일: E_FIELD_SCALES = [1.0, 10.0, 100.0]
E_FIELD_SCALES = [1, 2, 3, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50]  # 시뮬레이션할 E-field 배수 리스트
...
try:
    ...
    if E_UNIT_SCALE != 1.0:
        ...
        print(f"  단위 변환: E_UNIT_SCALE = {E_UNIT_SCALE:.0e} (FEM → V/m)")
...
def set_extracellular_field():
    ...
    # E-field 스케일 팩터 설정 (정규화 없이 원본에 직접 곱함)
    if not hasattr(set_extracellular_field, '_e_scale_calculated'):
        # 정규화 없이 E_FIELD_SCALE을 직접 사용
        ...
        final_peak = measured_peak * E_FIELD_SCALE
        print(f"  [정규화 없음] 원본 최대 E-field: {measured_peak:.6f} V/m")
        print(f"  E-field 스케일: {E_FIELD_SCALE:.1f}배")
        print(f"  최종 최대 E-field: {final_peak:.6f} V/m")
...
# 여러 E-field 스케일로 시뮬레이션 반복 실행
for scale_idx, E_FIELD_SCALE in enumerate(E_FIELD_SCALES):
    print("\n" + "="*70)
    print(f"시뮬레이션 {scale_idx + 1}/{len(E_FIELD_SCALES)}: E-field 스케일 = {E_FIELD_SCALE:.1f}배 (원본 기준)")
    print("="*70)
    ...
    # tqdm 진행률 바 초기화
    pbar = tqdm(total=total_steps, desc=f"E-field 시뮬레이션 ({E_FIELD_SCALE:.1f}x)", unit="step", unit_scale=True, ncols=100)
    ...
    # --- 검증 체크리스트 ---
    print("\n" + "="*60)
    print(f"검증 체크리스트 (E-field 스케일: {E_FIELD_SCALE:.1f}배)")
    print("="*60)
    ...
    all_Ez_max = np.max(np.abs(E_field_values[1, :, :]))
    measured_peak = max(all_Ex_max, all_Ez_max)
    print(f"  전체 데이터 최대 E-field (원본): {measured_peak:.6f} V/m")
    if hasattr(set_extracellular_field, '_e_scale'):
        final_peak = measured_peak * set_extracellular_field._e_scale
        print(f"  E-field 스케일: {set_extracellular_field._e_scale:.6f} (정규화 없음, 원본 기준 {E_FIELD_SCALE:.1f}배)")
        print(f"  최종 최대 E-field: {final_peak:.6f} V/m")

    # 기대 범위: tES/tACS 수준은 보통 0.1 ~ 2 V/m
    final_peak_check = measured_peak * E_FIELD_SCALE
    if final_peak_check < 1e-2:  # 0.01 V/m 미만
        print("  ⚠️ 경고: 최종 E-field가 너무 작습니다 (< 0.01 V/m).")
    elif final_peak_check < 0.1:  # 0.1 V/m 미만
        print("  ⚠️ 주의: 최종 E-field가 작습니다 (< 0.1 V/m).")
    elif final_peak_check > 20:  # 20 V/m 초과
        print("  ⚠️ 경고: 최종 E-field가 매우 큽니다 (> 20 V/m).")
    else:
        print("  ✅ 최종 E-field 크기가 정상 범위입니다 (0.1 ~ 2 V/m, tES/tACS 수준)")
    ...
    try:
        ...
        print(f"\n--- 5. 결과 플롯 생성 및 저장 (E-field 스케일: {E_FIELD_SCALE:.1f}배) ---")
        ...
        if PLOT_TYPE in ['all', 'both']:
            ...
            if MODEL_TYPE == 'allen' and ALLEN_CELL_ID:
                ...
                output_filename = f'simple_neuron_response_all{scale_suffix}.png'
        ...
        if PLOT_TYPE in ['single', 'both']:
            ...
            if np.any(cycle_mask):
                ...
                if MODEL_TYPE == 'allen' and ALLEN_CELL_ID:
                    ...
                    output_filename_single = f'simple_neuron_response_single_cycle{scale_suffix}.png'
